...
def approve_and_build(project, root, *, keep_backing=True):
    """Called only by the explicit approval action, inside the command history."""
    from . import vocal_mix as V, io as IO
    root = Path(root)
    project.alignment['review'] = dict(required=True, approved_fingerprint=fingerprint(project),
                                       keep_backing=bool(keep_backing))
    ...
    return True, 'Timing approved and karaoke audio built.'
